...
// ── Types (mirrored from web frontend) ──────────────────────────
...
export type AdminOverview = {
  ...
  courses: { total: number; published: number; draft: number; pending: number; archived: number };
  lessons: number;
  enrollments: number;
...
export async function refreshSession(): Promise<AuthSession | null> {
  ...
  refreshPromise = (async () => {
    try {
      ...
      await setAccessToken(payload.data.access_token);
...
export const api = {
  ...
  entry: (language: string, slug: string) =>
    request<DictionaryEntry>(`/api/dictionary/${language}/${slug}`),
  dashboard: (language = "en") =>
    request<DashboardSummary>(`/api/dashboard?language=${language}`),
  ...
  grammar: (level = "") =>
    request<GrammarTopic[]>(`/api/grammar?language=en${level ? `&level=${level}` : ""}`),
  review: (reviewId: string, rating: string) =>
    request<{ review_id: string; rating: string; next_review_in_days: number }>(`/api/review/${reviewId}`, {
      method: "POST",
  ...
  // Learner course path
  courses: (language = "en") =>
    request<CourseCard[]>(`/api/courses?language=${language}`),
  courseDetail: (slug: string) =>
    request<CourseDetail>(`/api/courses/${encodeURIComponent(slug)}`),
  ...
  completeLesson: (lessonId: string, score?: number) =>
    request<LessonDetail>(`/api/lessons/${lessonId}/complete`, {
      method: "POST",
  ...
  exam: (examId: string) =>
    request<LearnerExam>(`/api/exams/${examId}`),
  ...
  admin: {
    ...
    updateCourseStatus: (courseId: string, status: string) =>
      request<{ id: string; status: string }>(`/api/admin/courses/${courseId}`, {
        method: "PATCH",
        body: JSON.stringify({ status }),
      }),
    reviewCourse: (courseId: string, action: "approve" | "reject", note = "") =>
      request<AdminCourse>(`/api/admin/courses/${courseId}/review`, {
        method: "POST",
        body: JSON.stringify({ action, note }),
      }),
    languages: () => request<AdminLanguage[]>("/api/admin/languages"),
    updateLanguage: (languageId: string, isActive: boolean) =>
      request<{ is_active: boolean }>(`/api/admin/languages/${languageId}`, {
        method: "PATCH",
  ...
  instructor: {
    ...
    setSkills: (courseId: string, skills: { skill: string; note: string }[]) =>
      request<{ skill: string; note: string }[]>(`/api/instructor/courses/${courseId}/skills`, {
        method: "PUT",
        body: JSON.stringify({ skills }),
      }),
    lessons: (courseId: string) =>
      request<InstructorLesson[]>(`/api/instructor/courses/${courseId}/lessons`),
    createLesson: (courseId: string, input: {
      title: string;
      summary?: string;
      content: string;
      video_url?: string;
    ...
    updateLesson: (lessonId: string, input: {
      title?: string;
      summary?: string;
      content?: string;
      video_url?: string;
    ...
    lessonQuestions: (lessonId: string) =>
      request<AuthorQuestion[]>(`/api/instructor/lessons/${lessonId}/questions`),
    ...
    exams: (courseId: string) =>
      request<InstructorExam[]>(`/api/instructor/courses/${courseId}/exams`),
    createExam: (courseId: string, input: {
      title: string;
      description?: string;
      pass_score?: number;
      duration_minutes?: number;
    ...
    updateExam: (examId: string, input: {
      title?: string;
      description?: string;
      pass_score?: number;
      duration_minutes?: number;
    ...
    examQuestions: (examId: string) =>
      request<AuthorQuestion[]>(`/api/instructor/exams/${examId}/questions`),
